...
ENV:
  CITYVIBE_S2S_AUTH_ENABLED=true
  CITYVIBE_S2S_TOKEN=...
...
def verify_s2s_token(token: str | None) -> None:
    """Проверка токена. No-op если CITYVIBE_S2S_AUTH_ENABLED=false."""
    f = flags()
    if not f.s2s_auth:
        return
    if not token:
...
class S2SAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if path in f.s2s_skip_paths or any(path.startswith(p.rstrip("/") + "/") for p in f.s2s_skip_paths if p.endswith("/*")):
            return await call_next(request)
