...
def flags() -> InfraFlags:
    ...
    return InfraFlags(
        ...
        request_id=env_bool("CITYVIBE_REQUEST_ID_ENABLED", True),
        # S2S / idempotency / rate-limit / outbox — по умолчанию ВЫКЛ (local-friendly)
        s2s_auth=env_bool("CITYVIBE_S2S_AUTH_ENABLED", False),
        s2s_token=env_str("CITYVIBE_S2S_TOKEN", "dev-s2s-token"),
        s2s_header=env_str("CITYVIBE_S2S_HEADER", "X-Service-Token"),
        s2s_skip_paths=tuple(p.strip() for p in skip.split(",") if p.strip()),
        idempotency=env_bool("CITYVIBE_IDEMPOTENCY_ENABLED", False),
        idempotency_ttl_sec=env_int("CITYVIBE_IDEMPOTENCY_TTL_SEC", 86400),
        idempotency_header=env_str("CITYVIBE_IDEMPOTENCY_HEADER", "Idempotency-Key"),
        rate_limit=env_bool("CITYVIBE_RATE_LIMIT_ENABLED", False),
        rate_limit_default=env_int("CITYVIBE_RATE_LIMIT_DEFAULT", 60),
        rate_limit_window_sec=env_int("CITYVIBE_RATE_LIMIT_WINDOW_SEC", 60),
        upload_validation=env_bool("CITYVIBE_UPLOAD_VALIDATION_ENABLED", True),
        upload_max_bytes=env_int("CITYVIBE_UPLOAD_MAX_BYTES", 5 * 1024 * 1024),
