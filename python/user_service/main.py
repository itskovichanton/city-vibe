...
Запуск:
  python main.py
  (из каталога python/user_service или через IntelliJ Run Configuration)
"""
...
def _bootstrap_sys_path() -> None:
    ...
    Нужно, чтобы работали импорты:
      - user_service.* / python.user_service.*  (пакеты монорепо)
      - python.libs.*
    Фреймворк mybootstrap_* уже лежит в site-packages/src.
    """
    service_dir = Path(__file__).resolve().parent  # .../python/user_service
...
from python.user_service.src.user_service.app import UserServiceApp  # noqa: E402
